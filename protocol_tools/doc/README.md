...
`"urn:iso:std:iso:20022:tech:xsd:pacs.008.001.11"`:
    ...
    #> tidy -xml -indent <ISO20022/pacs.008.001.11.xsd | head
    No warnings or errors were found.
...
Extracting the value of the `targetNamespace` from the schema:
    ...
    1> {ok,Bin} = file:read_file("ISO20022/pacs.008.001.11.xsd").
    2> Pacs008 = decode_XML:document(Bin).
    3> Trim008 = decode_XML:trim_namespace(Pacs008).
    ...
    7> Schema008 = decode_XML:schema(Trim008).
    ...
    10> GenMsg008 = decode_XML:generate_from_schema(<<"Document">>,Schema008).
    11> {element,<<"Document">>,MsgAtt008,Content008} = GenMsg008.
    12> BinMsg008 = decode_XML:encode(
          { element,
            <<"Document">>,
...
example PACS.008 message XML data:
    ...
    #> tidy -xml -indent <ISO20022/pacs008.xml | head -10
    No warnings or errors were found.
...
In the `ISO20022/pacs.008.001.11.xsd` file there are two type definitions,
...
Testing the generators:
    ...
    8> decode_XML:generate_from_schema(
